...
| Search content injects instructions or exfiltrates data | Only literature and citation roles receive live web search and their sandboxes stay read-only; the workflow grants no command network; runtime types reject other roles and wildcard host patterns; approval remains `never` | Search sources can serve hostile content. Operators must avoid secrets in model context and review any future command-network host policy. |
...
| A path escapes the managed root | Run IDs and artifact paths use closed patterns; export rejects symbolic-link components and writes by staging rename; migration rejects source symlinks | An operator with host access can replace the managed root while the process is down. Filesystem permissions remain an operator duty. |
| Local Impeccable tooling follows a hostile checkout path or executes checkout text | Linux descriptor-pinned filesystem helpers restrict reads, writes, and removals to regular entries under an opened repository directory, including across parent swaps; child processes receive fixed argv without a shell; the edit agent uses a workspace-write sandbox with approvals disabled; package-defined validation stays disabled unless the maintainer opts in | Impeccable can modify repository files by design. A same-account process with checkout write access can rewrite a Svelte session manifest and redirect Accept to another in-repository `.svelte` file, but it already has authority to edit that file directly. The helpers fail closed without Linux `/proc/self/fd`; they do not isolate mutually hostile tools under one account. Maintainers must review generated edits. |
| Another local page connects to the Impeccable live server or reads screenshot bytes | The loopback server requires a random UUID token for the bootstrap script and later messages; injected code keeps the token in lexical scope; source routes use contained regular-file reads; screenshot contrast analysis runs in Node rather than the page context | Processes running as the same OS account can inspect process state or repository files. The live token does not isolate mutually hostile processes under one account. |
...
- the host, service account, Codex authentication context, and data-root parent
  directory belong to the operator;
