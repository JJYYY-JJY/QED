...
export function persistCache(cwd, cache) {
  ...
  try {
    ...
    writeContainedFile(cwd, target, JSON.stringify(cache), { encoding: 'utf-8' });
    return true;
  } catch {
    return false;
  }
...
export function writeAuditLog(env, entry, cwd = process.cwd()) {
  ...
  const fromEnvironment = typeof target === 'string' && target.length > 0;
  ...
  try {
    ...
    if (fromEnvironment) {
      ...
      appendContainedFile(baseCwd, expanded, line, { encoding: 'utf-8' });
    }
    return true;
  } catch {
    return false;
