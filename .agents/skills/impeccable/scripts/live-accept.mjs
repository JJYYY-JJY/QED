...
Output (JSON):
  ...
  if (!/^[0-9a-f]{8}$/.test(id)) { console.error('Invalid --id'); process.exit(1); }
...
function findSessionFile(id, cwd) {
  ...
  for (const dir of searchDirs) {
    let absDir;
    try {
      absDir = resolveContainedPath(cwd, path.join(cwd, dir), {
        allowMissing: false,
        allowRoot: true,
        type: 'directory',
      });
    ...
    const result = searchDir(absDir, marker, seen, 0, cwd);
    if (result) {
      const content = readContainedFile(cwd, result, 'utf-8');
...
function searchDir(dir, query, seen, depth, cwd) {
  if (depth > 5) return null;
  try {
    resolveContainedPath(cwd, dir, {
      allowMissing: false,
      allowRoot: true,
      type: 'directory',
    });
  ...
  for (const entry of entries) {
    ...
    try {
      const content = readContainedFile(cwd, filePath, 'utf-8');
  ...
  for (const entry of entries) {
    ...
    const result = searchDir(path.join(dir, entry.name), query, seen, depth + 1, cwd);
