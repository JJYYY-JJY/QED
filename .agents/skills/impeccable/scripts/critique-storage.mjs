...
import { readContainedFile, writeContainedFile } from './lib/safe-fs.mjs';
...
export function writeSnapshot({ slug, meta, body, cwd = process.cwd(), now = new Date() }) {
  ...
  writeContainedFile(cwd, filePath, `${front}\n${body.trim()}\n`, { encoding: 'utf-8' });
  return filePath;
}
...
export function readLatestSnapshot(slug, { cwd = process.cwd() } = {}) {
  ...
  const body = readContainedFile(cwd, latest, 'utf-8');
...
export function readTrend(slug, { limit = 5, cwd = process.cwd() } = {}) {
  ...
  return slice.map((file) => parseFrontmatter(readContainedFile(cwd, file, 'utf-8')));
}
