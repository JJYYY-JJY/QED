...
const DETECTOR_SCRIPT = path.join(path.dirname(fileURLToPath(import.meta.url)), 'detect.mjs');
...
function latestCritique(cwd) {
  try {
    ...
    const text = readContainedFile(cwd, path.join(dir, newest), 'utf-8');
...
function bundledDetectionSignals(cwd, scan) {
  if (!Array.isArray(scan?.targets) || scan.targets.length === 0) {
    return { attempted: false, findings: [], error: null };
  }
  const targets = [];
  for (const target of scan.targets) {
    try {
      targets.push(resolveContainedPath(cwd, target, {
        allowMissing: false,
        allowRoot: true,
      }));
    } catch {
      // Repository-controlled symlinks and escaped paths are not scan inputs.
    }
  }
  if (targets.length === 0) return { attempted: false, findings: [], error: null };
  try {
    const output = execFileSync(process.execPath, [DETECTOR_SCRIPT, '--json', ...targets], {
      cwd,
      encoding: 'utf-8',
...
export async function gatherSignals(cwd = process.cwd(), { withDetection = false } = {}) {
  const ctx = loadContext(cwd);
  const git = gitSignals(cwd);
  const scan = scanTargets(cwd, git);
  const signals = {
    setup: {
  ...
  if (withDetection) signals.detection = bundledDetectionSignals(cwd, scan);
  return signals;
}

async function cli() {
  const signals = await gatherSignals(process.cwd(), {
    withDetection: process.argv.slice(2).includes('--with-detection'),
  });
