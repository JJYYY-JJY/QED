...
function readRawConfigFile(cwd, filePath) {
  if (!fs.existsSync(filePath)) return { exists: false, malformed: false, raw: null };
  try {
    return { exists: true, malformed: false, raw: JSON.parse(readContainedFile(cwd, filePath, 'utf-8')) };
  } catch {
...
function readRawHookConfig(cwd, opts = {}) {
  const unified = readRawConfigFile(cwd, opts.local ? getLocalConfigPath(cwd) : getConfigPath(cwd)).raw;
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  const existingRaw = readRawConfigFile(cwd, filePath).raw;
  ...
  writeContainedFile(cwd, filePath, JSON.stringify(next, null, 2) + '\n');
  return filePath;
}
...
function statusReport(cwd) {
  const shared = readRawConfigFile(cwd, getConfigPath(cwd));
  const local = readRawConfigFile(cwd, getLocalConfigPath(cwd));
...
export function repairHookManifests(cwd) {
  ...
  for (const target of HOOK_MANIFEST_TARGETS) {
    ...
    resolveContainedPath(cwd, dest, { allowMissing: true });
    if (sharedDest) resolveContainedPath(cwd, sharedDest, { allowMissing: true });

    if (sharedDest && fileHasImpeccableHookMarker(cwd, sharedDest)) {
      pruneImpeccableHookFromManifest(cwd, dest);
    ...
    if (fs.existsSync(dest)) {
      try {
        next = mergeHookManifests(JSON.parse(readContainedFile(cwd, dest, 'utf-8')), fresh);
      } catch {
        const backup = `${dest}.bak`;
        writeContainedFile(cwd, backup, readContainedFile(cwd, dest));
        result.backups.push(backup);
      }
    }

    const serialized = `${JSON.stringify(next, null, 2)}\n`;
    const current = fs.existsSync(dest) ? safeReadText(cwd, dest) : null;
    if (current === serialized) {
      result.already.push(target.provider);
      continue;
    }
    writeContainedFile(cwd, dest, serialized);
    result.written.push(target.provider);
  }
  return result;
}

function safeReadText(cwd, filePath) {
  try {
    return readContainedFile(cwd, filePath, 'utf-8');
  } catch {
    return null;
  }
}
...
function fileHasImpeccableHookMarker(cwd, filePath) {
  if (!fs.existsSync(filePath)) return false;
  let parsed;
  try {
    parsed = JSON.parse(readContainedFile(cwd, filePath, 'utf-8'));
  } catch {
    return false;
  }
...
function pruneImpeccableHookFromManifest(cwd, manifestPath) {
  resolveContainedPath(cwd, manifestPath, { allowMissing: true });
  if (!fileHasImpeccableHookMarker(cwd, manifestPath)) return false;
  let parsed;
  try {
    parsed = JSON.parse(readContainedFile(cwd, manifestPath, 'utf-8'));
  } catch {
    return false;
  }
  ...
  if (Object.keys(next).length === 0) {
    removeContainedFile(cwd, manifestPath, { force: true });
  } else {
    writeContainedFile(cwd, manifestPath, `${JSON.stringify(next, null, 2)}\n`);
  }
  return true;
}
...
function reset(cwd) {
  ...
  for (const filePath of [getConfigPath(cwd), getLocalConfigPath(cwd)]) {
    try {
      const raw = readRawConfigFile(cwd, filePath).raw;
      if (!raw || typeof raw !== 'object' || Array.isArray(raw) || (!('hook' in raw) && !('detector' in raw))) continue;
      const { hook, detector, ...rest } = raw;
      if (Object.keys(rest).length === 0) {
        removeContainedFile(cwd, filePath, { force: true });
      } else {
        writeContainedFile(cwd, filePath, JSON.stringify(rest, null, 2) + '\n');
      }
  ...
  for (const filePath of [getCachePath(cwd), getPendingPath(cwd)]) {
    try {
      if (removeContainedFile(cwd, filePath, { force: true })) {
        removed.push(path.relative(cwd, filePath) || filePath);
      }
    } catch { /* ignore */ }
  }
