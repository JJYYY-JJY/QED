...
import {
  ...
  isPathInsideOrEqual,
  makeContainedTemporaryDirectory,
  readContainedFile,
...
function recordManualEditActivity(type, details = {}) {
  ...
  if (DEBUG_MANUAL_EDIT_EVENTS) {
    try {
      ...
      appendContainedFile(process.cwd(), filePath, JSON.stringify(entry) + '\n', { encoding: 'utf-8' });
    } catch {
...
function statOrNull(filePath) {
  ...
  try {
    const root = contextFileRoot(filePath);
    const safePath = resolveContainedPath(root, filePath, { allowMissing: false, type: 'file' });
...
function contextFileRoot(filePath) {
  const repoRoot = PROJECT_CONTEXT.repoRoot || process.cwd();
  return isPathInsideOrEqual(repoRoot, filePath)
    ? repoRoot
    : (CONTEXT_DIR || path.dirname(filePath));
}

function readContextFile(filePath) {
  return readContainedFile(contextFileRoot(filePath), filePath, 'utf-8');
}
  ...
  return (req, res) => {
    ...
    if (p === '/annotation' && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        try {
          writeContainedFile(process.cwd(), absPath, Buffer.concat(chunks));
        } catch (err) {
          res.writeHead(500, { 'Content-Type': 'application/json' });
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (p === '/design-system/raw') {
        ...
        try {
          const content = readContextFile(mdPath);
      ...
      if (mdStat) {
        try {
          response.parsed = parseDesignMd(readContextFile(mdPath));
        } catch (err) {
      ...
      if (jsonStat) {
        try {
          response.sidecar = JSON.parse(readContextFile(jsonPath));
        } catch (err) {
...
function shutdown() {
  ...
  state.leaseTimer = null;
  if (state.sessionDir) {
    try { removeContainedDirectory(process.cwd(), state.sessionDir, { force: true }); } catch {}
  }
...
if (existingRecord?.info) {
  ...
  try {
    ...
    try { removeLiveServerInfo(process.cwd()); } catch {}
  }
}
...
ensureContainedDirectory(process.cwd(), annotRoot);
state.sessionDir = makeContainedTemporaryDirectory(process.cwd(), annotRoot, 'session-');
