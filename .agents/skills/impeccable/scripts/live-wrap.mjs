...
The agent should insert variant HTML at insertLine.`);
  ...
  targetFile = resolveContainedPath(process.cwd(), targetFile, { allowMissing: false, type: 'file' });
  if (isGeneratedFile(targetFile, genOpts)) {
    console.error(JSON.stringify({
      error: 'file_is_generated',
      fallback: 'agent-driven',
      file: path.relative(process.cwd(), targetFile),
      hint: 'Source file is generated. Writing here gets wiped by the next build. See "Handle fallback" in live.md.',
    }));
    process.exit(1);
  }

  const content = readContainedFile(process.cwd(), targetFile, 'utf-8');
  ...
  if (useSvelteComponent) {
    ...
    writeContainedFile(process.cwd(), targetFile, newLines.join('\n'), { encoding: 'utf-8' });
...
function findFileWithQuery(query, cwd, genOpts = {}) {
  ...
  for (const dir of searchDirs) {
    ...
    let containedDir;
    try {
      containedDir = resolveContainedPath(cwd, absDir, {
        allowMissing: false,
        allowRoot: true,
        type: 'directory',
      });
    ...
    const result = searchDir(containedDir, query, seen, 0, { ...genOpts, cwd });
...
function searchDir(dir, query, seen, depth, genOpts) {
  ...
  for (const entry of entries) {
    ...
    try {
      const content = readContainedFile(genOpts.cwd, filePath, 'utf-8');
