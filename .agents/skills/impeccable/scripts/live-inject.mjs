 ...
 *
 * Usage:
 *   node live-inject.mjs --port PORT --token UUID   # Insert the live script tag
...
import { assertLiveBootstrapCredentials } from './live/browser-script-parts.mjs';
import {
...
Modes:
  ...
  --token UUID  Authenticate the live.js bootstrap request
...
Output (JSON):
  ...
  if (args.includes('--check')) {
    ...
    let cfg;
    try {
      cfg = JSON.parse(readContainedFile(process.cwd(), CONFIG_PATH, 'utf-8'));
    } catch (err) {
  ...
  const config = JSON.parse(readContainedFile(process.cwd(), CONFIG_PATH, 'utf-8'));
  ...
  if (args.includes('--remove')) {
    ...
    const results = resolvedFiles.map((relFile) => {
      ...
      const content = readContainedFile(process.cwd(), absFile, 'utf-8');
      ...
      writeContainedFile(process.cwd(), absFile, updated, { encoding: 'utf-8' });
      return {
        file: relFile,
  ...
  const rawPort = portIdx !== -1 ? args[portIdx + 1] : null;
  const port = typeof rawPort === 'string' && /^[1-9]\d*$/.test(rawPort)
    ? Number(rawPort)
    : NaN;
  ...
  try {
    ...
    console.error(JSON.stringify({ ok: false, error: 'invalid_token', message: err.message }));
    process.exit(1);
  }
...
export function ensureLiveGitIgnores(cwd = process.cwd()) {
  ...
  let existing = '';
  try { existing = readContainedFile(cwd, target.path, 'utf-8'); } catch (error) {
  ...
  if (updated !== existing) {
    writeContainedFile(cwd, target.path, updated, { encoding: 'utf-8' });
  }
...
export function resolveFiles(rootDir, config) {
  ...
  for (const pat of patterns) {
    if (path.isAbsolute(pat) || pat.split(/[\\/]+/).includes('..')) {
      throw new Error(`Configured live file escapes project root: ${pat}`);
    }
    if (!isGlob(pat)) {
