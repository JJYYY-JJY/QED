...
import {
  isPathInsideOrEqual as isContainedPath,
  readContainedFile,
...
export function loadContext(cwd = process.cwd(), options = {}) {
  ...
  const product = productPath ? safeRead(productPath, resolved) : null;
  const design = designPath ? safeRead(designPath, resolved) : null;
  return {
    hasProduct: !!product,
    product,
...
function discoverRootsForPattern(repoRoot, rawPattern) {
  ...
  if (!segments.length || path.isAbsolute(pattern) || segments.includes('..')) return [];
...
function readPackageWorkspaces(repoRoot) {
  const pkg = readJson(repoRoot, path.join(repoRoot, 'package.json'));
...
function readLernaWorkspaces(repoRoot) {
  const lerna = readJson(repoRoot, path.join(repoRoot, 'lerna.json'));
  return Array.isArray(lerna?.packages) ? lerna.packages : [];
}

function readPnpmWorkspaces(repoRoot) {
  try {
    const body = readContainedFile(repoRoot, path.join(repoRoot, 'pnpm-workspace.yaml'), 'utf-8');
    const patterns = [];
...
function readJson(root, filePath) {
  try {
    return JSON.parse(readContainedFile(root, filePath, 'utf-8'));
  } catch {
    return null;
  }
}
...
function firstExisting(dir, names) {
  for (const name of names) {
    ...
    try {
      resolveContainedPath(dir, abs, { allowMissing: false, type: 'file' });
      return abs;
    } catch {}
  }
  return null;
...
function safeRead(p, resolved) {
  try {
    const root = isContainedPath(resolved.repoRoot, p)
      ? resolved.repoRoot
      : resolved.contextDir;
    return readContainedFile(root, p, 'utf-8');
  } catch {
    return null;
  }
}
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(readContainedFile(cwd, path.join(cwd, '.impeccable', name), 'utf-8'));
