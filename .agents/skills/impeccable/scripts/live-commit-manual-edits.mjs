...
function snapshotRollbackFiles(cwd, files = null) {
  ...
  const rollbackFiles = Array.isArray(files)
  ...
  for (const relativeFile of rollbackFiles) {
    try {
      snapshot.set(relativeFile, {
        existed: true,
        content: readContainedFile(cwd, relativeFile, 'utf-8'),
      });
    } catch {}
  }
  return snapshot;
...
function changedFilesSinceSnapshot(cwd, snapshot, scopeFiles = null) {
  ...
  const scopedFiles = Array.isArray(scopeFiles)
    ? scopeFiles.map((file) => normalizeRollbackScopePath(cwd, file)).filter(Boolean)
    : null;
  ...
  for (const [relativeFile, before] of snapshot.entries()) {
    ...
    const current = readRollbackFile(cwd, relativeFile);
    if (before?.existed === false) {
      if (current.kind === 'file') changed.set(relativeFile, { file: relativeFile, kind: 'added' });
      if (current.kind === 'unsafe') changed.set(relativeFile, { file: relativeFile, kind: 'unsafe' });
      continue;
    }
    ...
    if (current.kind === 'unsafe') {
      changed.set(relativeFile, { file: relativeFile, kind: 'unsafe' });
      continue;
    }
...
function readRollbackFile(cwd, relativeFile) {
  try {
    return { kind: 'file', content: readContainedFile(cwd, relativeFile, 'utf-8') };
  } catch (err) {
...
function rollbackChangedFiles(cwd, snapshot, extraFiles = [], scopeFiles = []) {
  const scope = new Set((scopeFiles || [])
    .map((file) => normalizeRollbackScopePath(cwd, file))
    .filter(Boolean));
  const safeExtraFiles = (extraFiles || [])
    .map((file) => normalizeRollbackPath(cwd, file))
    .filter(Boolean);
  for (const relativeFile of safeExtraFiles) scope.add(relativeFile);
  const changed = changedFilesSinceSnapshot(cwd, snapshot, [...scope]);
  const byFile = new Map(changed.map((item) => [item.file, item]));
  for (const relative of safeExtraFiles) {
  ...
  for (const item of byFile.values()) {
    ...
    try {
      if (before?.existed !== false && typeof before?.content === 'string') {
        writeContainedFile(cwd, item.file, before.content, { encoding: 'utf-8' });
      } else if (before?.existed === false && item.kind === 'added') {
        removeContainedFile(cwd, item.file);
      } else {
...
function normalizeRollbackPath(cwd, file) {
  const relative = normalizeRollbackScopePath(cwd, file);
  if (!relative) return null;
  try {
    const absolute = resolveContainedPath(cwd, relative, { allowMissing: false, type: 'file' });
    if (isGeneratedFile(absolute, { cwd })) return null;
    return relative;
  } catch {
    return null;
...
function normalizeRollbackScopePath(cwd, file) {
  if (!file || typeof file !== 'string') return null;
...
export async function commitManualEdits({
  ...
  if (result.status === 'error') {
    const rollbackScope = mergeUniqueStrings(
      baseRollbackScope,
      collectApplyOwnedFiles(batch, cwd, result.files || []),
    );
