...
function requireSessionId(id) {
  if (typeof id !== 'string' || !SESSION_ID_RE.test(id)) {
    throw new Error('Invalid Svelte component session id');
  }
  return id;
}

function digestOriginalMarkup(markup) {
  return createHash('sha256').update(String(markup || ''), 'utf-8').digest('hex');
}
...
export function componentSessionDir(id, cwd = process.cwd()) {
  return resolveContainedPath(
    cwd,
    path.join(cwd, SVELTE_COMPONENT_ROOT, requireSessionId(id)),
    { allowMissing: true },
  );
}
...
export function ensureRuntimeHelper(cwd = process.cwd()) {
  const file = resolveContainedPath(cwd, SVELTE_RUNTIME_FILE, { allowMissing: true });
  if (fs.existsSync(file)) {
    return resolveContainedPath(cwd, file, { allowMissing: false, type: 'file' });
  }
  writeContainedFile(cwd, file, `export { mount, unmount } from 'svelte';\n`, { encoding: 'utf-8' });
  return file;
}
...
export function scaffoldSvelteComponentSession({
  ...
  ensureContainedDirectory(cwd, dir);
  ...
  const manifest = {
    ...
    originalMarkup,
    originalDigest: digestOriginalMarkup(originalMarkup),
  ...
  for (let n = 1; n <= count; n++) {
    const variantFile = resolveContainedPath(cwd, path.join(dir, `v${n}.svelte`), { allowMissing: true });
    if (!fs.existsSync(variantFile)) {
      writeContainedFile(cwd, variantFile, buildVariantStub(n, originalWithProps, contract), { encoding: 'utf-8' });
    } else {
      resolveContainedPath(cwd, variantFile, { allowMissing: false, type: 'file' });
    }
  }
...
export function scaffoldSvelteComponentInsertSession({
  ...
  const manifest = {
    ...
    anchorMarkup,
    originalDigest: digestOriginalMarkup(anchorMarkup),
    count,
    propContract: [],
    componentDir: path.relative(cwd, dir).split(path.sep).join('/'),
    runtimeModule: `/${SVELTE_RUNTIME_FILE}`,
  ...
  for (let n = 1; n <= count; n++) {
    const variantFile = resolveContainedPath(cwd, path.join(dir, `v${n}.svelte`), { allowMissing: true });
    if (!fs.existsSync(variantFile)) {
      writeContainedFile(cwd, variantFile, buildInsertVariantStub(n), { encoding: 'utf-8' });
    } else {
      resolveContainedPath(cwd, variantFile, { allowMissing: false, type: 'file' });
    }
  }
...
export function findSvelteComponentManifest(id, cwd = process.cwd()) {
  ...
  return readManifest(direct, cwd, id);
}

export function readManifest(manifestPath, cwd = process.cwd(), expectedId = null) {
  const safeManifestPath = resolveContainedPath(
    cwd,
    manifestPath,
    { allowMissing: false, type: 'file' },
  );
  const data = JSON.parse(readContainedFile(cwd, safeManifestPath, 'utf-8'));
  validateManifestIdentity(data, cwd, expectedId);
  return {
    ...data,
...
function validateManifestIdentity(manifest, cwd, expectedId = null) {
  if (!manifest || typeof manifest !== 'object' || Array.isArray(manifest)) {
    throw new Error('Invalid Svelte component manifest');
  }
  if (manifest.schemaVersion !== 1) {
    throw new Error('Invalid Svelte component manifest schema version');
  }
  const allowedKeys = new Set([
    ...
    ...(manifest.mode === 'insert'
      ? ['mode', 'insertLine', 'position', 'anchorStartLine', 'anchorEndLine', 'anchorMarkup']
      : ['sourceStartLine', 'sourceEndLine']),
  ]);
  if (Object.keys(manifest).some((key) => !allowedKeys.has(key))) {
    throw new Error('Invalid Svelte component manifest field');
  }
  const id = requireSessionId(manifest.id);
  if (expectedId !== null && id !== requireSessionId(expectedId)) {
    throw new Error('Svelte component manifest session id mismatch');
  }
  if (manifest.previewMode !== 'svelte-component') {
    throw new Error('Invalid Svelte component manifest preview mode');
  }
  if (
    typeof manifest.sourceFile !== 'string'
    || path.isAbsolute(manifest.sourceFile)
    || manifest.sourceFile.includes('\\')
    || path.extname(manifest.sourceFile).toLowerCase() !== '.svelte'
  ) {
    throw new Error('Invalid Svelte source file in component manifest');
  }
  resolveSourceFile(manifest.sourceFile, cwd);
  const expectedComponentDir = path
    .relative(cwd, componentSessionDir(id, cwd))
    .split(path.sep)
    .join('/');
  if (manifest.componentDir !== expectedComponentDir) {
    throw new Error('Svelte component manifest directory mismatch');
  }
  if (manifest.runtimeModule !== `/${SVELTE_RUNTIME_FILE}`) {
    throw new Error('Invalid Svelte component manifest runtime module');
  }
  if (
    !Array.isArray(manifest.propContract)
    || manifest.propContract.some((entry) => {
      if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return true;
      if (Object.keys(entry).sort().join(',') !== 'expr,placeholder,prop') return true;
      return (
        typeof entry.expr !== 'string'
        || typeof entry.placeholder !== 'string'
        || entry.placeholder !== `{${entry.expr}}`
        || typeof entry.prop !== 'string'
        || !/^[A-Za-z_$][\w$]*$/.test(entry.prop)
      );
    })
  ) {
    throw new Error('Invalid Svelte component manifest prop contract');
  }
  if (manifest.manifestPath !== undefined) {
    const expectedManifestPath = manifestPathForSession(id, cwd);
    if (path.resolve(manifest.manifestPath) !== path.resolve(expectedManifestPath)) {
      throw new Error('Svelte component manifest path mismatch');
    }
  }
  if (!Number.isInteger(manifest.count) || manifest.count < 1 || manifest.count > 8) {
    throw new Error('Invalid Svelte component manifest variant count');
  }
  if (typeof manifest.originalMarkup !== 'string' || !SHA256_RE.test(manifest.originalDigest || '')) {
    throw new Error('Invalid Svelte component manifest original digest');
  }
  if (digestOriginalMarkup(manifest.originalMarkup) !== manifest.originalDigest) {
    throw new Error('Svelte component manifest original digest mismatch');
  }
  if (manifest.mode === 'insert') {
    if (
      !Number.isInteger(manifest.insertLine)
      || manifest.insertLine < 1
      || !Number.isInteger(manifest.anchorStartLine)
      || manifest.anchorStartLine < 1
      || !Number.isInteger(manifest.anchorEndLine)
      || manifest.anchorEndLine < manifest.anchorStartLine
      || !['before', 'after'].includes(manifest.position)
      || manifest.anchorMarkup !== manifest.originalMarkup
    ) {
      throw new Error('Invalid Svelte insert manifest source range');
    }
  } else if (
    !Number.isInteger(manifest.sourceStartLine)
    || manifest.sourceStartLine < 1
    || !Number.isInteger(manifest.sourceEndLine)
    || manifest.sourceEndLine < manifest.sourceStartLine
  ) {
    throw new Error('Invalid Svelte component manifest source range');
  }
}
...
export function resolveSourceFile(sourceFile, cwd = process.cwd()) {
  ...
  return resolveContainedPath(cwd, sourceFile, { allowMissing: false, type: 'file' });
}
...
export function inlineSvelteComponentAccept(manifest, variantNum, paramValues = null, cwd = process.cwd()) {
  ...
  if (
    !Number.isInteger(selectedVariant)
    || selectedVariant < 1
    || selectedVariant > manifest.count
  ) {
  ...
  const { markup, cssLines } = parseSvelteComponentFile(readContainedFile(cwd, variantPath, 'utf-8'));
  if (manifest.mode === 'insert') {
  ...
  const sourceContent = readContainedFile(cwd, sourceFile, 'utf-8');
  ...
  const currentOriginalMarkup = sourceLines.slice(start, end + 1).join('\n');
  if (digestOriginalMarkup(currentOriginalMarkup) !== manifest.originalDigest) {
    throw new Error('Svelte source changed after variants were generated; refusing stale accept');
  }
  ...
  try {
    writeContainedFile(cwd, sourceFile, newLines.join('\n'), { encoding: 'utf-8' });
  } catch (err) {
...
function inlineSvelteComponentInsertAccept({
  ...
  const anchorStart = Number(manifest.anchorStartLine) - 1;
  const anchorEnd = Number(manifest.anchorEndLine) - 1;
  if (
    !Number.isInteger(anchorStart)
    || !Number.isInteger(anchorEnd)
    || anchorStart < 0
    || anchorEnd < anchorStart
    || anchorEnd >= sourceLines.length
  ) {
    return { handled: false, error: 'Invalid insert anchor range for ' + manifest.sourceFile, ...resultBase };
  }
  const currentAnchorMarkup = sourceLines.slice(anchorStart, anchorEnd + 1).join('\n');
  if (digestOriginalMarkup(currentAnchorMarkup) !== manifest.originalDigest) {
    throw new Error('Svelte source changed after variants were generated; refusing stale insert accept');
  }
...
export function removeSvelteComponentSession(id, cwd = process.cwd()) {
  try {
    const dir = componentSessionDir(id, cwd);
    removeContainedDirectory(cwd, dir, { force: true });
  } catch { /* non-fatal */ }
}
...
export function removeAllSvelteComponentSessions(cwd = process.cwd()) {
  ...
  let entries;
  try {
    entries = readContainedDirectory(cwd, root, { withFileTypes: true });
  } catch {
    return;
  }
  for (const entry of entries) {
    ...
    try {
      removeContainedDirectory(cwd, path.join(root, entry.name), { force: true });
    } catch { /* non-fatal */ }
  }
}
