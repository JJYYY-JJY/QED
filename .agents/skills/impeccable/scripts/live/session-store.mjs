...
export function createLiveSessionStore({ cwd = process.cwd(), sessionId } = {}) {
  ...
  ensureContainedDirectory(cwd, rootDir);
  ...
  function loadCachedOrRebuild(id) {
    ...
    const rebuilt = rebuildSnapshotFromJournal(journalPath, id, cwd);
  ...
  return {
    ...
    appendEvent(event) {
      ...
      if (!fs.existsSync(journalPath) && fs.existsSync(legacyJournalPath)) {
        writeContainedFile(cwd, journalPath, readContainedFile(cwd, legacyJournalPath));
      }
      ...
      appendContainedFile(cwd, journalPath, JSON.stringify(entry) + '\n', { encoding: 'utf-8' });
      ...
      writeSnapshot(snapshotPath, next, cwd);
      return next;
    },
    getSnapshot(id = sessionId, opts = {}) {
      ...
      const rebuilt = rebuildSnapshotFromJournal(journalPath, id, cwd);
      snapshotCache.set(id, rebuilt);
      writeSnapshot(snapshotPath, rebuilt.snapshot, cwd);
...
function rebuildSnapshotFromJournal(journalPath, id, cwd = process.cwd()) {
  let snapshot = baseSnapshot(id);
  const diagnostics = [];
  let nextSeq = 1;
  if (!fs.existsSync(journalPath)) return { snapshot, diagnostics, nextSeq };

  const lines = readContainedFile(cwd, journalPath, 'utf-8').split('\n');
...
function writeSnapshot(snapshotPath, snapshot, cwd = process.cwd()) {
  writeContainedFile(cwd, snapshotPath, JSON.stringify(snapshot, null, 2) + '\n', { encoding: 'utf-8' });
}
