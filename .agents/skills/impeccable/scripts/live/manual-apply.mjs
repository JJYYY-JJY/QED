...
export function createManualApplyController({
  ...
  function pruneStaleEvidence(cwdValue = projectCwd()) {
    ...
    const removed = [];
    let entries;
    try { entries = readContainedDirectory(cwdValue, dir, { withFileTypes: true }); } catch { return []; }
    for (const entry of entries) {
      ...
      try {
        removeContainedFile(cwdValue, fullPath, { force: true });
...
export function writeManualApplyEvidence(eventId, batch, cwd = process.cwd()) {
  ...
  writeContainedFile(cwd, evidencePath, JSON.stringify(batch, null, 2) + '\n', { encoding: 'utf-8' });
  return evidencePath;
}
...
export function normalizeManualApplyEvidencePath(evidencePath, cwd = process.cwd()) {
  ...
  try {
    return resolveContainedPath(cwd, fullPath, { allowMissing: true, type: 'file' });
  } catch {
    return null;
  }
}
...
export function removeManualApplyEvidence(evidencePath, cwd = process.cwd()) {
  ...
  try {
    return removeContainedFile(cwd, fullPath, { force: true });
  } catch {
    return false;
  }
}
...
export function snapshotApplyEventFiles(batch, cwd = process.cwd()) {
  ...
  for (const relativeFile of collectManualApplyFiles(batch, [], cwd)) {
    const absolute = resolveContainedPath(cwd, relativeFile, { allowMissing: true, type: 'file' });
    try {
      const exists = fs.existsSync(absolute);
      snapshot.set(relativeFile, {
        exists,
        content: exists ? readContainedFile(cwd, absolute, 'utf-8') : '',
      });
    } catch {
...
export function writeManualApplyTransaction({ cwd = process.cwd(), pageUrl = null, batch }) {
  ...
  writeContainedFile(cwd, file, JSON.stringify(transaction, null, 2) + '\n', { encoding: 'utf-8' });
  return transaction;
}
...
export function clearManualApplyTransaction(cwd = process.cwd(), transactionId = null) {
  ...
  try {
    return removeContainedFile(cwd, file, { force: true });
  } catch {
    return false;
  }
}
...
export function rollbackManualApplyTransaction({
  ...
  for (const item of transaction.files || []) {
    ...
    try {
      if (item.exists) {
        writeContainedFile(cwd, absolute, item.content || '', { encoding: 'utf-8' });
      } else {
        removeContainedFile(cwd, absolute, { force: true });
      }
...
function normalizeProjectFile(file, cwd = process.cwd()) {
  ...
  let absolute;
  try {
    absolute = resolveContainedPath(cwd, file, { allowMissing: true, type: 'file' });
  } catch {
    return null;
  }
...
export function rollbackApplySnapshot(
  ...
  for (const relativeFile of scope) {
    ...
    try {
      if (before.exists) {
        writeContainedFile(cwd, absolute, before.content, { encoding: 'utf-8' });
      } else {
        removeContainedFile(cwd, absolute, { force: true });
      }
      rolledBackFiles.push(relativeFile);
