...
export function buildCopyEditBatchPrompt(batch, { cwd = process.cwd() } = {}) {
  ...
  return [
    ...
    '- Only read or write regular files below the canonical repository root shown below. Never follow symbolic links, and never edit parent, sibling, home, or other external paths.',
    ...
    '- Do not run project-defined validation scripts unless IMPECCABLE_LIVE_ALLOW_PROJECT_VALIDATION=1. When enabled, scripts.impeccable:manual-edit-validate must pass after edits.',
...
export function runCopyEditPostApplyChecks({
  cwd = process.cwd(),
  files = [],
  allowProjectValidation = process.env.IMPECCABLE_LIVE_ALLOW_PROJECT_VALIDATION === '1',
} = {}) {
  const failures = [];
  const warnings = [];
  ...
  for (const relativeFile of uniqueFiles) {
    let file;
    try {
      file = resolveContainedPath(cwd, relativeFile, { allowMissing: false, type: 'file' });
    } catch {
      warnings.push({ file: relativeFile, reason: 'file_missing_or_outside_cwd' });
      continue;
    }
    let content = '';
    try { content = readContainedFile(cwd, file, 'utf-8'); } catch (err) {
  ...
  const validation = runManualEditValidationScript(cwd, { allowProjectValidation });
...
function runManualEditValidationScript(cwd, { allowProjectValidation = false } = {}) {
  const script = readManualEditValidationScript(cwd);
  if (!script) return null;
  if (!allowProjectValidation) {
    return {
      warning: {
        file: 'package.json',
        reason: 'manual_edit_validation_requires_opt_in',
      },
    };
  }
  const validation = spawnSync('npm', ['run', '--silent', 'impeccable:manual-edit-validate'], {
    cwd,
    encoding: 'utf-8',
    shell: false,
    timeout: 30_000,
...
function readManualEditValidationScript(cwd) {
  ...
  try {
    const pkg = JSON.parse(readContainedFile(cwd, pkgPath, 'utf-8'));
...
function applyMockWrites(env, cwd) {
  ...
  for (const [relativeFile, content] of Object.entries(writes)) {
    ...
    writeContainedFile(cwd, relativeFile, content, { encoding: 'utf-8' });
  }
}
...
function runCodex(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = buildCodexArgs({ cwd, env, resultPath });
  return runAgentProcess('codex', args, prompt, { cwd, env, logPath, timeoutMs });
}

export function buildCodexArgs({ cwd, env = {}, resultPath }) {
  const args = [
    'exec',
    '--cd', cwd,
...
function runClaude(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = buildClaudeArgs({ env, prompt });
...
export function buildClaudeArgs({ env = {}, prompt = '' } = {}) {
  const args = [
    '--print',
