 ...
 *
 * Injected into the user's page via an authenticated localhost script URL.
 * The server wraps this code with lexical bootstrap credentials.
 *
...
(function () {
  ...
  const IMPECCABLE_COMMAND = (IMPECCABLE_COMMAND_PREFIX || '/') + 'impeccable';
  ...
  // the lexical live bootstrap when it serves /live.js. The icons stack above
  ...
  const VOCAB = Array.isArray(IMPECCABLE_VOCAB) ? IMPECCABLE_VOCAB : [];
  const ICONS = {};
