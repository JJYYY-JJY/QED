...
1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. Setup has already run `context.mjs`; if that reported `NO_PRODUCT_MD` the project has no captured context yet, so lead the menu with `$impeccable init` as the top recommendation (one line on why) and still show the rest below; don't silently jump into init. Otherwise run the fixed command `node .agents/skills/impeccable/scripts/context-signals.mjs --with-detection` once and read its JSON, then lead with the **2-3 highest-value next commands**, each with a one-line reason pulled from the signals, followed by the full menu (the table above, grouped by category). The helper invokes the detector without a shell; never interpolate reported filenames into another command. **Never auto-run a command; the recommendation is a suggestion the user confirms.**
   ...
   **If `detection.attempted` is true and the project is not native, use the returned `detection.findings`; do not construct or run another command from `scan.targets`.** The fixed helper already ran the bundled detector over contained local paths without a shell. `scan.via` explains the selection: `git-changes`, `source-dir`, `html`, or `root`. Fold the findings into your picks: many quality / contrast hits → `audit` or `polish`; a specific slop family → the matching command (gradient text or eyebrows → `quieter` / `typeset`, flat or gray palette → `colorize`, and so on). If `detection.error` is non-null, skip those findings and recommend the user run `audit`; never block the suggestion on detection.
